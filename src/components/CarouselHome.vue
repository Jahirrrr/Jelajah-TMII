<template>
    <section class="max-w-full px-2 mx-auto container-home lg:max-w-screen-xl">
        <div
            class="home relative mx-auto mt-4 flex min-h-[34rem] w-full flex-col items-center justify-center overflow-hidden rounded-[2rem] bg-secondary py-24 shadow-2xl sm:min-h-[38rem] md:rounded-[3rem] lg:min-h-[42rem] lg:py-40">
            <img v-for="(tour, index) in topFiveRandomTours" :key="index" :src="`/assets/${tour.image.image}`"
                :alt="tour.image.alt || `gambar wisata ${tour.name}`" class="img-slide rounded-[2rem] md:rounded-[3rem]" loading="lazy"
                :class="{ active: index === currentSlide }" />

            <div v-for="(tour, index) in topFiveRandomTours" :key="'content-' + index"
                class="relative content z-[777] my-8 w-[88%] max-w-full px-3 text-light md:mx-auto lg:max-w-screen-lg lg:px-0 xl:max-w-screen-xl"
                :class="{ 'hidden': index !== currentSlide, 'active': index === currentSlide }">
                <div class="text-center text-wrap">
                    <h1
                        class="mb-4 text-3xl font-bold leading-tight tracking-wide text-center text-transparent uppercase text-stroke text-wrap font-poppins sm:text-5xl lg:text-7xl">
                        <span class="text-3xl font-poppins text-light sm:text-5xl lg:text-7xl">Jelajah</span><br />
                        {{ tour.name }}
                    </h1>
            ...
            <div class="absolute mx-auto mt-2 text-center typing-box md:left-4">
                <p class="font-bold line animation-typing font-inter text-light">JELAJAH TMII</p>
            </div>
